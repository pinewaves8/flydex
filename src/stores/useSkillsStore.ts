...
const BUILTIN_SKILLS: SkillDefinition[] = [
  {
    ...
    inputMarker: '',
    prompt:
      '【执行要求】请立即用 web_search MCP 工具搜索【用户指令】中的内容并返回结果，' +
      '禁止输出任何开场白、规则复述、确认或反问，直接开始搜索。规则：\n' +
      '1) 先只用人名/主体本身搜索（中文名、英文名各至少一次；英文名必须用裸名"名+姓"如 "Jingtian Wu"，严禁堆叠机构/院校名如 "Jingtian Wu Cornell"——实测会被同名干扰污染，裸名才能命中真实个人主页），严禁编造或假设其身份属性（职业/学校/专业/奖项/地区等）作为搜索限定词——用编造属性搜索只会返回同名干扰项；\n' +
      '2) 共搜索 4~6 次，连续 3 次无关键信息必须换更宽泛或另一种语言的关键词，不要用同一思路无限重试；\n' +
      '3) 必要时用 web_fetch 抓取关键链接（百科/新闻/个人主页）的完整正文以获取毕业年份、学校、专业等细节；\n' +
      '4) 每次搜索后先整合所有结果再回复，不要每调用一次工具就写一段；\n' +
      '5) 不同姓名的搜索结果严禁交叉引用/拼接，必须严格区分每个搜索结果对应的人物；\n' +
      '6) 信息来自搜索结果才可作为事实，未覆盖的信息必须如实写"未找到公开信息"；\n' +
      '7) 同一人物的多个结果要交叉比对属性（时间线/学校/专业/地域/成就）确认是否为同一人，不要轻易判定"同名不同人"；\n' +
      '8) 只输出最终结论，不要复述中间搜索过程；关键词不要拼错。',
    enabled: true,
  },
  {
    name: 'review',
...
export const useSkillsStore = create<SkillsState>((set, get) => ({
  ...
  execute: (name: string, userInput: string) => {
    ...
    if (skill.name === 'web-search') {
      // 任务前置：用户指令放最前、规则放后，避免模型被长规则淹没后反问"搜索什么"（MiniMax 对任务后置不稳定）
      return `【用户指令】${userInput}\n\n${prefix}`
    }
