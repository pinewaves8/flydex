...
> ⚠️ **本文是 2026-08-25 的计划快照，不是当前规范。**
> 项目已进入「复用 codex harness」阶段，当前约束与进度见 **`AGENTS.md`**
> （§2.4 收敛进度 / §2.5 实测陷阱），仍未处理的问题见 `docs/known-issues.md`。
> 下文的任务编号与阶段划分保留作历史记录。
...
> **7.2 Agent 层三件套（2026-09-04 已完成 + GUI 实测通过）**：
> 全部为前端实现（复用现有 runCodex/stopCodex 通道，后端无需变更）。
> **7.2.1 角色分派**：新增 `types/subagent.ts` 定义三角色预设（General 通用 / Explore 快读只读代码分析 / Plan 只读研究出方案），各带指令模板 + 默认沙箱（Explore/Plan 强制只读）+ 推荐模型说明；任务卡新增「角色」下拉（选中自动预填模板 + 切只读沙箱）与「模型」下拉（从模型列表每子代理独立选择，默认跟随全局）。
> **7.2.2 团队协调**：任务卡新增「前置任务」下拉；runAll 两阶段启动——先无依赖任务，前置完成后把其 lastAgent 结论注入下游指令并自动启动（SendMessage 语义，输出区记录「▸ 已接收上游 XXX 结论」）。
> **7.2.3 后台常驻**：新增 `stores/useSubagentStore.ts`（backgroundRunning/Total），面板收起后任务继续后台运行；ChatPanel「子代理」按钮显示运行数徽章，点击可重新展开。
...
> **超时兜底（54d05d4）**：场景 C 实测发现模型发出的复杂 PowerShell 命令（含单引号/正则，如 `Get-ChildItem -Recurse | Where-Object {...}` 递归扫描）被 codex app-server 转义环节破坏（引号错乱）导致 PowerShell 挂起等续行**永不返回**、后台任务一直 running。新增 15 分钟执行超时自动终止兜底（5s 轮询，stop + 标记 error + 输出区提示），防止后台任务永久卡死。
...
> **7.3 决策记录（2026-09-04）：不做 ML 分类器，做确定性规则扩充。**
> **结论**：权限 auto 不引入 ML（本地分类器 / 主模型预判），改为把确定性危险规则做厚。不是"难度大"——三条路径技术上都可实现；核心原因是**权限决策是安全关键场景，错判成本不对称**：误拦截=多弹一次审批（低成本），误放行=危险命令真执行（高风险），而 ML 是概率性判断，在安全边界上不可审计归因、无法承担兜底。
> **关键澄清**：Claude Code 的 auto 权限本质不是"本地 ML 模型"，而是"主模型每步预判 + 内置 deny 列表强制兜底"。flydex 规则引擎（deny > allow > ask + 危险白名单）架构上已对齐其安全边界，缺的是规则覆盖面而非 ML。
> **三路径对比**：A 复用主模型预判（最贴近原版，但慢+费 token、不可审计、可能误放行；长期可选默认关）；B 本地小模型分类器（低延迟但精度不足以承担权限决策，假阴性=漏洞，不建议）；C 确定性规则扩充（零延迟、可审计、无假阴性，长尾靠 ask 兜底，✅ 推荐）。
> **7.3 实施范围**：① 危险命令白名单覆盖做全（删除/覆盖、系统目录、注册表、凭据文件、网络外连、脚本执行器等）；② 加启发式模式匹配（正则/令牌，确定性非 ML：如 `Format-*`、`diskpart`、`reg delete`、`Remove-Item -Recurse` 等危险前缀直接 deny）。
...
- **Tasks API** ✅ 已完成 7.4.2（2026-09-04）
...
> **7.4.2 Tasks API（已完成 + 单测通过，17cebe6，待 GUI 实测）**：
> 后端：models/task.rs（Task：status[todo/in_progress/done/cancelled] + priority + labels + projectId/sessionId 关联，兼容旧 snake_case）；services/tasks.rs（~/.flydex/tasks.json 持久化 CRUD，按项目过滤 + 按更新时间倒序，支持 data_dir 注入测试隔离）；commands/task.rs（task_list/create/update/delete）。单测 7/7，全量 lib 37 passed。
> 前端：types/task.ts + taskService.ts（invoke 封装）；features/tasks/TaskPanel.tsx（状态过滤[全部/待办/进行中/已完成/已取消]、新建/内联编辑表单、状态循环切换[待办→进行中→完成]、删除、关联会话标题展示）；ChatPanel 顶栏 Tasks 按钮 + 挂载（自动关联当前项目 + 当前会话）。
