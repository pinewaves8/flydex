...
const REVIEW_INSTRUCTION: &str = "【代码审查模式】你正在以资深工程师视角审查代码变更。diff 内容已保存在工作目录下的 .flydex-review.diff 文件中，请先用读取工具读取该文件，再输出结构化审查报告。输出格式要求：1) 第一行输出一行总评；2) 之后每行一个问题，严格格式：级别|文件:行号|问题概述|修改建议，级别只能取【严重】【警告】【建议】【好评】之一（好评用于肯定做得好的点），文件与行号必须来自 diff 中的真实位置；3) 即使没有严重问题也至少要给出一条好评或说明；4) 最后一行输出：审查总结：xxx。5) 不要重复输出任何内容，每条问题与总结只出现一次。";
